...
- **WhatsApp is never the first-touch channel by default.** It's used sparingly, mainly once a prior touch has established some relationship or the dossier explicitly notes context that makes it appropriate.
...
- **Unipile** would power actual LinkedIn message delivery once a draft is approved, and read the reply thread back in for `reply-to-pipeline`.
- **Smartlead or Instantly** would power email sequence delivery and open/click tracking, matching the "always draft, review in the UI, hit start manually" pattern this skill's approval gate is adapted from.
...
3. WhatsApp only applies if this isn't the first touch, or the dossier explicitly notes context that makes it appropriate. Skip it by default.
...
WhatsApp: not drafted (first touch, no prior relationship)
...
- WhatsApp is never the default first-touch channel.
